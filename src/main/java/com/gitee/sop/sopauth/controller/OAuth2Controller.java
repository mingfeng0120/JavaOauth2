...
public class OAuth2Controller {
     ...
     * @return
     */
    @RequestMapping("open.auth.token.app")
    @ResponseBody
    public FetchTokenResult fetchToken(HttpServletRequest request) {
    	System.out.println(request.getParameterMap());
    	System.out.println(request.getParameter("biz_content"));
    	String par = request.getParameter("biz_content");
    	if(StringUtils.isEmpty(par)){
    		return null;
    	}
    	FetchTokenParam param = JSONObject.parseObject(par, FetchTokenParam.class);
    ...
    @RequestMapping("open.auth.token")
